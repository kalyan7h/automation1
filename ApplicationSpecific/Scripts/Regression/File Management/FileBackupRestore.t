		...
		[+] if(QuickenWindow.Exists(5))
			...
			[+] if(QuickenBackup.Exists(10))
				...
				[+] else
					...
					[ ] QuickenBackup.SetActive()
					[ ] QuickenBackup.BackupFile.SetText(sExpectedFilePath)
				[ ] 
...
[+] testcase Test14_VerifyPreferencesForAutomaticBackup() appstate QuickenBaseState
	[+] // Variable declaration
		[ ] STRING sPreferenceType,sAutomaticBackup,sBackupPath, sItem
		[ ] LIST OF ANYTYPE  lsListOfFiles
					...
					[+] if(bMatch==TRUE)
						...
						[+] for(i=1;i<=Val(sAutomaticBackup)*3;i++)
							[+] if (QuickenWindow.Exists(10))
								...
								[+] if(QuickenBackupReminder.Exists(3))
									[ ] QuickenBackupReminder.SetActive()
						...
						[+] if(QuickenRestore.Exists(2))
							...
							[+] if(iCount==Val(sAutomaticBackup))
									...
									[ ] // Verify File exist in specific location with name and date
									[ ] 
									[ ] lsListOfFiles=GetDirectoryListing(sFileManagementFilePath+"\Backup\")
									[ ] 
									[+] for each sItem in lsListOfFiles
										[ ] bMatch = MatchStr ("*{ sBackupPath}*" ,sItem)
										[+] if (bMatch)
											[ ] break
									[ ] 
									[+] if(bMatch)
...
[+] //#############  Verify the navigation for Quicken Backup Preferences Dialog Window  ###################################
	[ ] // ********************************************************
	[+] // TestCase Name:	 Test15_VerifyPreferencesForManualBackup ()
		[ ] //
		[ ] // DESCRIPTION:
...
[+] testcase Test15_VerifyPreferencesForManualBackup() appstate QuickenBaseState
	[+] // Variable declaration
		[ ] STRING sPreferenceType,sManualBackup,sBackupPath , sItem
		[ ] LIST OF ANYTYPE  lsListOfFiles
		[ ] 
		[ ] sPreferenceType="Backup"
		[ ] sManualBackup="2"
		...
		[+] if(iValidate==PASS)
			...
			[+] if(bMatch)
				...
				[+] if(Preferences.ManualBackupAfterRunningQuicken.Exists(3))
					[ ] Preferences.ManualBackupAfterRunningQuicken.SetText(sManualBackup)
					[ ] 
				[+] else
				...
				[+] for(i=1;i<=Val(sManualBackup);i++)
					...
					[+] if(i==Val(sManualBackup))
						[+] if(BackupConfirmation.Exists(2))
							...
							[+] if(QuickenBackup.Exists(2))
								[ ] QuickenBackup.BackUpNow.Click()
							[+] if(QuickenBackup.DuplicateBackupFile.Exists(3))
								[ ] QuickenBackup.DuplicateBackupFile.SetActive()
								[ ] QuickenBackup.DuplicateBackupFile.Yes.Click()
								[ ] WaitForState(QuickenBackup.DuplicateBackupFile , FALSE , 5)
							[+] if(BackupConfirmation.Exists(2))
								[ ] BackupConfirmation.OK.Click()
								[ ] WaitForState(BackupConfirmation , FALSE , 5)
						[+] else
				...
				[+] if(bMatch)
					...
					[+] if (ImportExportQuickenFile.Exists(10))
						[ ] ImportExportQuickenFile.SetActive()
						[ ] ImportExportQuickenFile.FileName.SetText(sBackupPath)
						[ ] 
						[ ] ImportExportQuickenFile.OK.Click()
						[ ] 
						[ ] 
	...
	[+] // for(i=1;i<=5;i++)
		...
		[+] // if(QuickenBackupReminder.Exists(3))
			...
			[+] // if(DuplicateBackupFile.Exists(3))
				...
				[ ] // 
				[ ] // 
			[+] // if(BackupConfirmation.Exists(15))
				[ ] // 
				[ ] // BackupConfirmation.SetActive()
				[ ] // BackupConfirmation.OK.Click()
				[ ] // 
...
[+] testcase Test11_VerifyRestoreFunctionalityByRestoringExistingFileWithOverwriteOption() appstate none
	...
	[+] if(iResult==PASS)
		...
		[+] if(QuickenWindow.Exists(5))
			[ ] QuickenWindow.SetActive()
			[ ] 
			[ ] iValidate= QuickenBackup(AUT_DATAFILE_PATH+ "\File Management data\","{sTempFile}-{sDate}.QDF-backup")
			[+] if(iValidate==PASS)
				[ ] // Verify File exist in specific location with name and date
				[+] if(FileExists(sFilePath))
					[ ] QuickenWindow.SetActive()
				...
				[+] else
					...
					[ ] 
			[+] else
				[ ] ReportStatus("Backup file",FAIL,"Backup done i.e. {sFilePath}")
			[ ] 
		[+] else
			[ ] ReportStatus("Verify Quicken Exists. ", FAIL , "Quicken does not exist.") 
		[ ] 
...
[+] testcase Test12_VerifyRestoreFunctionalityByRestoringExistingFileWithCopyOption() appstate none
	...
	[+] if(FileExists("{sBackupCopyFilePath}.QDF"))
		[ ] DeleteFile("{sBackupCopyFilePath}.QDF")
		[ ] 
	[ ] 
	[ ] iResult=OpenDatafile(sFileName , sFileManagementFilePath)
	[+] if(iResult==PASS)
...
[+] testcase Test16_VerifyInvalidLoacationForManualBackup() appstate QuickenBaseState
	...
	[+] if (QuickenWindow.Exists(5))
		...
		[+] if(QuickenBackup.Exists(5))
			...
			[ ] QuickenBackup.SetActive()
			[ ] // Enter Backup file path and file name
			[ ] QuickenBackup.BackupFile.SetText(sBackupPath + sFileName )
			[ ] // Click on Backup Now button
			[ ] QuickenBackup.BackUpNow.Click()
		[ ] 
...
[+] testcase Test18_VerifyRestoreAndOverwriteFor2012BackupFile() appstate QuickenBaseState
	[+] // Variable declaration
		...
		[ ] STRING sFilePath=AUT_DATAFILE_PATH+ "\FileIO"
	...
	[ ] 
	[ ] iValidate= OpenDataFile("FIleIO",sFilePath)
	[+] if(iValidate==PASS)
		[ ] ReportStatus("Open data file",PASS,"FileIO.QDF is opened")
		[ ] iValidate=NULL
			...
			[+] if(RestoreOpenFile.Exists(2))
				...
				[+] if(RestoreOpenFile.Exists(2))
					...
					[+] if(ConvertYourData.Exists(30))
						...
						[ ] 
					[+] else
						[ ] ReportStatus("Verify ConvertYourData dialog",FAIL,"ConvertYourData dialog did not appear")
				[+] else
					[ ] ReportStatus("verify RestoreOpenFile dialog",FAIL,"RestoreOpenFile dialog did not open")
				[ ] 
				[ ] 
			[+] else
				[ ] QuickenRestore.Yes.Click()
	...
	[+] else
		[ ] ReportStatus("Open data file",FAIL,"FileIO.QDF is not opened")
	[ ] 
	[ ] 
	[ ] 
	[ ] 
...
[+] testcase Test19_VerifyRestoreAndCreateACopyFor2012BackupFile() appstate QuickenBaseState
	...
	[+] if(iValidate==PASS)
		...
		[+] if(iValidate==PASS)
				...
				[+] if(RestoreOpenFile.Exists(2))
					...
					[+] if(CopyQuickenFileBrowser.Exists(3))
						...
						[+] else
							...
							[ ] 
				[+] else
					[ ] ReportStatus("Verify RestoreOpenFile dialog",FAIL,"RestoreOpenFile dialog is not opened")
				[ ] 
			[+] else
				[ ] QuickenRestore.Yes.Click()
			[ ] 
			[ ] 
		[+] else
			[ ] ReportStatus("Verify QuickenRestoreNavigation function ",FAIL,"QuickenRestoreNavigation function failed")
	[+] else
		[ ] ReportStatus("Open data file",FAIL,"FileIO.QDF is not opened")
	[ ] 
	[ ] 
...
[+] testcase Test20_VerifyValidationMessageOfDataFileHavingMaxFileName() appstate QuickenBaseState
	...
	[+] if(QuickenWindow.Exists(20))
		...
		[+] if (ImportExportQuickenFile.Exists(10))
			...
			[ ] 
			[ ] // 2015-08-17 KalyanG: added 'if' condition before clicking on the dialog
			[ ] // Reason: Dialog does not appear after clicking on OK button. Ref Build QW15 R8 and 25.1.1.3
			[+] if (ImportExportQuickenFile.Exists(2))
				[ ] ImportExportQuickenFile.SetActive()
				[ ] ImportExportQuickenFile.Cancel.Click()
			[ ] 
		[+] else
			[ ] ReportStatus("Verify Create Quicken File",FAIL,"Create Quicken File dialog is not opened")
	[+] else
...
[+] testcase Test21_VerifyBackupOfDataFileHavingMaxFileName() appstate QuickenBaseState
	[+] // Variable declaration
		...
		[ ] STRING sLongFileName="Quicken Data File having max number of characters in File name for verification of Quicken backup functionality Quicken"
		[ ] sFilePath=ROOT_PATH+"\"
		[ ] 
	...
	[+] if(iResult == PASS)
		...
		[ ] iResult=CloseQuicken()
		[+] if(iResult == PASS)
			[ ] ReportStatus("Verify Quicken exit action for large char data file ",PASS,"Data file with long length file name is closed successfully ")
			[ ] iResult=LaunchQuicken()
			[+] if(iResult == PASS)
				...
				[+] if(QuickenBackup.Exists(5))
					[ ] 
					[ ] QuickenBackup.SetActive()
					[ ] // Taking backup by clicking on Change link as QW-3081 is not resolved
					[ ] QuickenBackup.Change.Click()
					[+] if(QuickenBackup.DlgEnterBackupFileName.Exists(3))
						[ ] QuickenBackup.DlgEnterBackupFileName.SetActive()
						[ ] QuickenBackup.DlgEnterBackupFileName.FileTextField.SetText(sFilePath+sLongFileName)
						[ ] QuickenBackup.DlgEnterBackupFileName.OKButton.Click()
						[ ] QuickenBackup.BackUpNow.Click()
						...
						[ ] 
					[+] else
						[ ] ReportStatus("Verify DlgEnterBackupFileName dialog",FAIL,"DlgEnterBackupFileName did not open")
						[ ] ReportStatus("Backup Data File having long length file name", FAIL ,"Data File: {sLongFileName} is not backed up successfully.")
						[ ] 
				[+] else
					[ ] ReportStatus("Verify Quicken Backup dialog",FAIL,"Quicken Backup dialog did not open")
				[ ] 
				[ ] 
			[+] else
				[ ] ReportStatus("Verify Launch Quicken",FAIL,"Quicken is not launched")
		[+] else
			[ ] ReportStatus("Verify Quicken exit action for large char data file",FAIL,"Data file with long length file name is not closed successfully")
	[+] else
...
[+] testcase Test22_VerifyRestoreOfDataFileHavingMaxFileName() appstate QuickenBaseState
	[+] // Variable declaration
		...
		[ ] STRING sLongFileName="Quicken Data File having max number of characters in File name for verification of Quicken backup functionality Quicken"
		[ ] sFilePath=ROOT_PATH+"\"
		[ ] STRING sBackupFileName = "TempFile"
		[ ] STRING sTarget= AUT_DATAFILE_PATH+"\"+"FileIO"+"\{sBackupFileName}.QDF-backup"
		[ ] 
						...
						[+] if(iCreate==PASS)
							...
							[+] if(iNavigate==PASS)
								...
								[+] if(DuplicateBackupFile.Yes.Exists(5))
									...
									[ ] DuplicateBackupFile.Yes.Click()
								[ ] 
								[+] if(BackupConfirmation.Exists(18))
...
[+] testcase Test36_VerifyInvalidLoacationForRestore() appstate none
	[+] // Variable declaration
		...
		[ ] sFileName="Export"
		[ ] sBackupPath="Y:\Intuit\Backup\"
		[ ] sExpectedMessage="The drive that you specified is invalid. Please enter a valid drive and try again."
		[ ] 
	[ ] //Check the Quicken Existence 
	[+] if (QuickenWindow.Exists(5))
		[ ] QuickenWindow.SetActive()
			...
			[+] if (ImportExportQuickenFile.Exists(10))
				...
				[+] if(QuickenRestore.Exists(2))
					...
					[+] if(bCaption)
						...
						[+] if(iNavigate==PASS)
							[ ] bStatus=QuickenRestore.RestoreBackup.IsEnabled()
							[+] if(bStatus==FALSE)
							...
							[ ] 
							[ ] QuickenRestore.SetActive()
							[ ] QuickenRestore.BackupFilePath.SetText(sFileWithPath)
							[ ] sleep(1)
							[ ] QuickenRestore.SetActive()
							[ ] bStatus=QuickenRestore.RestoreBackup.IsEnabled()
							[+] if(bStatus)
								[ ] ReportStatus("Verify Restore backup button is disabled",PASS,"Restore backup button is enabled")
							[+] else
								[ ] ReportStatus("Verify Restore backup button is disabled",FAIL,"Restore backup button is disabled if backup file is selected")
								[ ] 
							[ ] QuickenRestore.SetActive()
...
[+] //############# Verify the restore functionality by "Restoring Currently Open File".###############################
	...
	[ ] // ********************************************************
	[ ] 
[+] testcase Test09_VerifyTheRestoreFunctionalityByRestoringCurrentlyOpenFile() appstate QuickenBaseState					
	[+] //--------------Variable Declaration-------------
		[ ] STRING sExpectedFilePath ,sTempFile
		[ ] STRING sBackup=".QDF-backup"
		[ ]   sDateFormat="yyyy-mm-dd"
		[ ]   sDate=ModifyDate(0,sDateFormat)
		[ ] sTempFile =RandStr("AAA")
		...
		[+] if(QuickenWindow.Exists(5))
			...
			[+] if(QuickenBackup.Exists(10))
				...
				[+] if(DuplicateBackupFile.Exists(5))
					...
					[ ] DuplicateBackupFile.Yes.Click()
				[ ] 
				[+] if(BackupConfirmation.Exists(25))
					[ ] BackupConfirmation.SetActive()
					[ ] BackupConfirmation.OK.Click()
					[ ] // Verify File exist in specific location with name and date
					[ ] ReportStatus("Verify Backup",PASS,"{sFilePath} backup done successfully")
					[ ] 
					[+] if(FileExists(sExpectedFilePath))
						[ ] QuickenWindow.SetActive()
						[ ] QuickenWindow.File.Click()
						...
						[+] if (QuickenRestore.Exists(10))
							...
							[+] for( iCounter=0;iCounter< iListCount ;++iCounter)
								[ ] print(iCounter)
								[ ] sActual = QwAutoExecuteCommand("LISTBOX_GETFULLROW", sHandle,  "{iCounter}")
								[ ] print("sActual: {sActual}")
								[ ] print("sFileManagementFileName: {sFileManagementFileName}")
								[ ] bMatch = MatchStr("*{sFileManagementFileName}*", sActual)
								[ ] print("bMatch: {bMatch}")
								[+] if (bMatch)
...
[+] testcase Test17_VerifyFileMenu() appstate QuickenBaseState
	[+] // Variable declaration
		...
		[ ] 
	[ ] 
	[ ] OpenDataFile(RPMNavigationDataFile,AUT_DATAFILE_PATH + "\DataFile")
